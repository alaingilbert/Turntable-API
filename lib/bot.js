...
(function() {
  var Bot, Errors, WebSocket, crypto, events, http, net, querystring;
  ...
  Bot = (function() {
    ...
    Bot.prototype.connect = function(roomId) {
      if (!/^[0-9a-f]{24}$/.test(roomId)) {
        return this.emit('error', new RangeError("Invalid roomId: cannot connect to '" + roomId + "'"));
      } else {
      ...
      return http.get(options, function(res) {
        ...
        return res.on('end', function() {
          ...
          if (data[0]) {
            ...
            return _this.emit('error', new Errors.ConnectionError('Error parsing server response'));
          }
        });
      }).on('error', function(e) {
        _this.log("whichServer error: " + e);
        return _this.emit('error', new Errors.ConnectionError(e));
      });
    };
    ...
    Bot.prototype.onError = function(e) {
      return this.emit('error', new Errors.SocketError(e));
    };
    ...
    Bot.prototype.maintainPresence = function() {
      ...
      if (this._isConnected && (Date.now() - activity) > this.disconnectInterval) {
        ...
        return this.emit('error', new TimeoutError('No response from server'));
      } else {
        return this.updatePresence();
      }
    };
